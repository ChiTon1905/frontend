        ...
        const fetchBooklayouts = async (e) => {
            try {
                const response = await axios.get(`http://127.0.0.1:8000/api/booklayouts/${id}`)
                    .then(respone => {
                        setBooklayouts(respone.data);
                    })
                console.log('axios', Booklayouts)
            } catch (error) {
                console.error('Error fetching:', error);
            }
        };

        document.title = 'Booklayout';
        fetchBooklayouts();
    }, [id]);

    console.log('Booklayouts', Booklayouts)
                ...
                <div className="container px-5 py-24 mx-auto">
                    <div className="flex flex-col justify-start w-full mb-20">
                        <h2 className="text-xl text-yellow-500 tracking-widest font-medium title-font uppercase mb-1">
                            {Booklayouts.data.attributes.name}</h2>
                        <hr className='mt-5 text-gray-600 w-auto '>
                        </hr>
                    </div>
                    <div className="flex flex-wrap -m-4">
                        {Booklayouts.data.attributes.books.map((product) => (
                            <div className="lg:w-1/4 md:w-1/2 p-4 w-full mb-4 cursor-pointer"
                             key={product.id} >
                                ...
                                <div className="mt-4">
                                    <h3 className="text-gray-500 text-xs tracking-widest title-font mb-1 uppercase">{ Booklayouts.data.attributes.name }</h3>
