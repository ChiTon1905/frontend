...
const Category = () => {
  ...
  const { category, setCategory, filterData, setFilterData, deleteCategory } = useCategory()
  ...
  const columns = [
    ...
    {
      name: 'name',
      selector: (row) => row.attributes.cat_name,
      sortable: true,
      sortFunction: (a, b) => {
        // Custom sorting function for the 'name' column
        return a.attributes.cat_name.localeCompare(b.attributes.cat_name);
      },
    },
    {
      name: 'action',
      ...
      cell: (row) => {
        ...
        const handleEditClick = () => {
          navigate(`/admin/category-update/${row.id}`);
        };
    ...
    if (e === '') {
      ...
      const filtered = category.filter((item) => {
        return (
          ...
          item.attributes.cat_name.toLowerCase().includes(e.toLowerCase())
        );
      });
