...
const Promotion = () => {
  ...
  const { promotions, setPromotions, filterData, setFilterData, deletePromotion } = usePromotions()
  ...
  const columns = [
    ...
    {
      name: 'discount',
      selector: (row) => `${row.attributes.discount * 100}%`,
      sortable: true,
    },
    {
      name: 'start_date',
      selector: (row) => format(new Date(row.attributes.start_date), 'dd-MM-yyyy'),
      sortable: true,
    },
    {
      name: 'end_date',
      selector: (row) => format(new Date(row.attributes.end_date), 'dd-MM-yyyy'),
      sortable: true,
    },
    {
      name: 'action',
      ...
      cell: (row) => {
        ...
        const handleEditClick = () => {
          navigate(`/admin/promotion-update/${row.id}`);
        };
