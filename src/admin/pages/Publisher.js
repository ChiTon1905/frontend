...
const Publisher = () => {
  ...
  const { publishers, setPublishers, filterData, setFilterData, deletePublisher } = usePublishers()
    ...
    },
    {
      ...
      cell: (row) => {
        ...
        const handleEditClick = () => {
          navigate(`/admin/publisher-update/${row.id}`);
        };
  ...
  const handleSearch = (e) => {
    setSearch(e)
    if (e === '') {
      setFilterData(publishers)
    } else {
      const filtered = publishers.filter((item) => {
        return (
  ...
  const handleCreate = () => {
    navigate('/admin/publisher-create')
  }
